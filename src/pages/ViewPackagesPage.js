...
import { Box, Container, IconButton, Stack, TableFooter, TablePagination, Typography, useTheme } from '@mui/material';
...
function createData(TrackingNum, Name, Description, Status) {
  return { TrackingNum, Name, Description, Status };
}
...
export default function ViewPackagesPage() {
  ...
  React.useEffect(() => {
    ...
    if (packages !== undefined) {
      const tempRows = [];
      packages.map((item) => {
        tempRows.push(createData(item.TrackingNumber, item.ItemName, '', item.ItemStatus));
        return null;
      });
      setRows(tempRows);
    }
  }, [packages, fetchPackages, currentUser, rows.length, value]);

  return (
    <>
      <Helmet>
        <title> Packages | Snip & Ship </title>
      </Helmet>
      <Container>
